...
var bars = iCanvas.createNewCtrl(function () {
    ...
    this.onRender(function (x, y) {
        ...
        for (var i in this.items) {
            ...
            var _dy = this.axis.getTopByValue(_single.l > 0 ? (_single.s + _single.l) : _single.s);//in case the height may be negtive numbers
            var _h = _exp("{" + Math.abs(_single.l) + "}").MUL(_u).c;
            var _singleBar;
            if (this.bars[i]) {
                _singleBar = this.bars[i];
                _singleBar.applyWH(undefined, _h);
                _singleBar.applyMargin(undefined,_dy);
            }
            else {
                _singleBar = iCanvas.ctrls.testRect(this.wrapper, this.singleW, _h, _dx, _dy, this.backC, { obj: this.GROUP, ctrl: "ctrl_g" });
                this.bars[i] = _singleBar;
                //deal with events
...
var PathLine = iCanvas.createNewCtrl(function () {
    ...
    this.onRender(function (x, y) {
        ...
        if (this.changed) {
            var _seed = 0;
            var _path;
            if (this.assets["Present_path"]) {
                _path = this.assets["Present_path"];
            }
            else {
                _path = iCanvas.ctrls.path(this.wrapper, 100, this.lineWidth, 0, 0, this.c);
                _path.setPosition(_Cons.POSITION.ABSOLUTE);
                this.assets["Present_path"] = _path;
            }
            _path.points = [];
            for (var i in this.items) {
                ...
                if (_path.points[_seed]) {
                    _path.points[_seed].x = _dx;
                    _path.points[_seed].y = _dy;
                }
                else {
                ...
                if (this.assets[i]) {
                    _node = this.assets[i];
                    _node.applyMargin(_dx, _dy);
                }
                else {
                    _node = iCanvas.ctrls.circle(this.wrapper, this.radius, this.lineWidth, _dx, _dy, this.c);
