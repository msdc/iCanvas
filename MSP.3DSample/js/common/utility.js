...
if (!window.requestAnimationFrame) {
    window.requestAnimationFrame = (window.webkitRequestAnimationFrame ||
                                    ...
                                    function (callback) {
                                        return window.setTimeout(callback, 5000 /*~ 1000/60*/);
                                    });
}
...
/**
 * Get a random number between the given two.
 * @param {number}  num1 the first given number
 * @param {number}  num2 the second given number
 * @return {number}
 */
window.utils.getRandom = function (num1, num2) {
...
/**
 * delegation implementation - Observer design model
 * @param {object}  sender, the source object of the delegation
 * @method {function}  attach, to attach a function
 * @method {function} notify, execute all attached methods
 */
window.utils.evtWrapper = function (sender) {
    this._sender = sender;
    this._listeners = [];
}

window.utils.evtWrapper.prototype.attach = function (handler) {
    if (typeof handler == "function") {
        this._listeners.push(handler);
    }
}

window.utils.evtWrapper.prototype.notify = function () {
